...
async def start_game_and_proxy():
    # 使用独立用户数据目录
    user_data_dir = os.path.abspath("./edge_profile")
    
    subprocess.Popen([
        r"C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe",
        f"--user-data-dir={user_data_dir}",
        "--proxy-server=127.0.0.1:23410",
...
async def start_mitm():
    # 创建 mitmproxy 配置
    opts = Options(
    listen_host='127.0.0.1',  # 关键修改：仅限本机访问
    listen_port=23410,
    ssl_insecure=True,
)
...
if __name__ == "__main__":
    asyncio.run(start_game_and_proxy())
